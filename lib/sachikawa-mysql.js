...
function shachikawaMysql(docJson) {
  return async function(appRoot, config, tableList) {
    ...
    for (const table of tables) {
      ...
      if (name.toUpperCase() === '_COMMENT') { // comment表作为特殊注释表单独读取全表数据
        if (docJson) {
          const comments = await sequelize.query('SELECT * FROM _COMMENT', { type: sequelize.QueryTypes.SELECT, raw: true });
          _.set(docJson, [ 'comments', database ], comments);
        }
      }
...
function onDocNotice(dbName, docJson) {
  _.set(docJson, [ 'databases', dbName ], _.get(docJson, [ 'databases', dbName ], {}));
  const dbObj = docJson.databases[dbName];
...
function getTableComment(comment) {
  comment = _.trimStart(comment.replace(/\r\n/g, ' ').replace(/\n/g, ' '));
  let group = null;
  if (comment[0] === '#') {
    const idx = comment.indexOf('#', 1);
    if (idx >= 0) {
      group = _.trim(comment.substring(1, idx));
      comment = _.trim(comment.substring(idx + 1));
    }
  }
